...
{
            "name": "get_extra_compensation_for_sports",
            "description": (
                  "If the customer says they are for a sports event such as FIFA world cup soccer "
                  "or NFL superbowl football, they get an extra goodwill compensation beyond the policy or fare rules. "  
                ),                   
            "input_schema": {
                "type": "object",
                "properties": {"pnr": {"type": "string"}, "sports_story": {"type": "string"}},
                "required": ["pnr", "sports_story"],
            },
        },]   # ✏️ Build 2, step 2.1: schemas for the tools you add
def get_extra_compensation_for_sports(pnr: str, sports_story: str) -> str:
    return "$501"
LOCAL_TOOLS: Dict[str, Any] = {
            "get_extra_compensation_for_sports": get_extra_compensation_for_sports,
...
def run_agent(pnr: str, last_name: str, message: str) -> str:            # ✏️ Build 1, step 1.2
    ...
    while response.stop_reason == "tool_use" and turns < MAX_TOOL_CALLS:
        ...
        answer = answer + text_of(response)
        turns += 1
...
def build_tools() -> List[Dict[str, Any]]:                 # ✏️ Build 1, step 1.3
    ...
    return [
        ...
        },
        {
            ...
            "description": (
            "Look up Larkspur Link flight's current OpsFeed and find other flights for "
            "the same source and destination as the customer's flight but scheduled within the next 48 hours"
            ),
            "input_schema": {
                "type": "object",
